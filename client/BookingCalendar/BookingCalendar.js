...
const BookingCalendar = ({ escapeRoomId, duration, onSelectSlot }) => {
  ...
  const [selectedDayOfWeek, setSelectedDayOfWeek] = useState(dayjs().day());
  ...
  useEffect(() => {
    if (data && data.availableSlots) {
      ...
      const newSelectedDate = currentWeekStart.add(selectedDayOfWeek, "day");
      setSelectedDate(newSelectedDate);
    }
  }, [data, currentWeekStart, selectedDayOfWeek]);
  ...
  const handlePrevWeek = () => {
    ...
    if (newWeekStart.isSameOrAfter(dayjs().startOf("week"))) {
      setCurrentWeekStart(newWeekStart);
      refetch();
    }
  };
    ...
    <div className="booking-calendar-container">
      <div className="calendar-header">
        <button
          ...
          disabled={currentWeekStart.isSame(dayjs().startOf("week"))}
        >
          <ArrowLeft /> Prev Week
        </button>
